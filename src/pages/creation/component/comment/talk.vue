<template>
  <el-container class="talk-container">
    <el-row class="bar" justify="space-between" align="middle">
      <span class="statistic">{{ "共计：" + (props.statistic["talk" + mode + (currentSelect === 'comment'?'':'Sub')] || 0) }}</span>
    ...
    <el-row class="body">
      <Reply v-if="mode === 'Reply' && currentSelect === 'comment'" :creation="'talk'"></Reply>
      <ReplySub v-if="mode === 'Reply' && currentSelect === 'sub-comment'" :creation="'talk'"></ReplySub>
      <Replied v-if="mode === 'Replied' && currentSelect === 'comment'" :creation="'talk'"></Replied>
      <RepliedSub v-if="mode === 'Replied' && currentSelect === 'sub-comment'" :creation="'talk'"></RepliedSub>
    </el-row>
  </el-container>
</template>
