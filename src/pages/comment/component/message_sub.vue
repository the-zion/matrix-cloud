<template>
  <el-container class="reply-sub-container">
    ...
    <el-row v-for="(item,index) in data" class="each" :key="item.id">
      <el-space class="title-block"
                @click="goToPage(item['creationType'] === 1?'article':'talk', {id:item['creationId']})">
        <el-tag type="info" effect="light">
          {{ item['creationType'] === 1 ? "文章" : "讨论" }}
        </el-tag>
        ...
        <el-row class="footer" justify="space-between">
          <el-space>
            ...
            <el-space class="icon-block" @click="commentRemove(index,item)"
                      v-if="item['uuid'] === uuid || item['rootUser'] === uuid">
              <el-icon class="icon">
                <Delete/>
              </el-icon>
...
let statistic = ref(0)
let baseUrlMap = {
  1: article.value.baseUrl,
  3: talk.value.baseUrl,
}
...
function getData() {
  ...
  loading.value = true
  getDataLock = true
  post("/v1/get/user/subcomment/replied/list", {page: currentPage}).then(function (reply) {
...
function getCreationIntroduce() {
  ...
  list.value.forEach(function (item) {
    if (!set[item["creationId"]]) {
      endpoints.push(baseUrlMap[item["creationType"]] + item["creationAuthor"] + "/" + item["creationId"] + "/introduce")
